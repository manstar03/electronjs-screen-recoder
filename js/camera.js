  ...
  navigator.mediaDevices
    ...
    .then(function (mic) {
      ...
      mediaRecorder = new MediaRecorder(output, {
        ...
        mimeType: "video/webm;codecs=h264", //codecs=vp9
      });
...
function updateCamera(id) {
  if (id != "disabled") {
    constraints = {
      audio: false,
      video: {
        deviceId: id, width: { max: 640 },
        height: { max: 480 }
      },
    };
    //FIXME camera.html DOM permission denied error when user forbid the camera access.
...
chrome.storage.sync.get(["flip"], function (result) {
  if (!result.flip) {
    if (typeof (WindowCanvasFreeDraw) === "undefined") {
...
chrome.storage.sync.get(["camera"], function (result) {
  if (result.camera != 0) {
    ...
    constraints = {
      audio: false,
      video: { deviceId: result.camera, width: 640, height: 480 },
    };
  } else {
    constraints = {
      audio: false,
